...
public class PropertyTest {
    ...
    @Test
    fun testPropertyException() {
        sut = Property(JSONObject())
        assertEquals("",sut.getPropertyId())
        assertEquals("",sut.getPropertyName())
        assertEquals(0,sut.getSegmentRules().length())
        assertEquals("",sut.getPropertyValue())
